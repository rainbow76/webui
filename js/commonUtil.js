var vObject = null;
var renameObj = null;
var navigation="<MenuRoot1 id=\"menuroot\"> <Menu id=\"home\" text=\"Home from text\" url=\"home.aspx\"></Menu><Menu id=\"projects\" text=\"Projects\" url=\"projects.aspx\"><SubMenu id=\"sub1\" text=\"Sub One\" url=\"subone.aspx\"></SubMenu><SubMenu id=\"sub2\" text=\"Sub Two\" url=\"subtwo.aspx\"></SubMenu><SubMenu id=\"sub3\" text=\"Sub Two\" url=\"subthree.aspx\"></SubMenu></Menu><Menu id=\"abc\" text=\"ABC\" url=\"http://abc.com\"></Menu></MenuRoot1>";
function start() {
    var $obj = $("#navigation");
    $obj.children().remove();
...
function publishModified(){
    var dummyElement = $("#headerNav > .treeview");
    $('#headerNav').empty();
    $('#headerNav').append(dummyElement);
    var rootElement = $("#navigation > li");
    var parent = $("#headerNav > .treeview");
    createMenuToggler(parent, rootElement);
}

function generateXML(){
    var menuRoot = $("#navigation > li > ul");
    
}
function publish(){
   $('#headerNav').empty();
    $('#headerNav').append("<h2>Preview</h2>");
    //Append menu
    //var div = $('<div/>');
    var ul = $('<ul class=\"treeview\"/>');
    var li = $('<li/>').html("<a style=\"padding-left: 10px;\">Menu</a><div class=\"previewMenu\"></div>");
    ...
    navigationElem.appendTo("#headerNav");


    $('.hitarea', $('#headerNav')).next().css("display", "none");
    $('.hitarea', $('#headerNav')).removeClass('collapsable-hitarea');
    $('.hitarea', $('#headerNav')).addClass('expandable-hitarea');
}
...
$('.hitarea', $('#previewCon')).click(function () {
...
$(function () {
    $('#view-source').click(function () {
        ...
        headTag = $('<html/>').append(headTag.html().replace("commonUtil", "previewUtil"));
    ...
    $('#view-complete-source').click(function () {
        ...
        var html = $('#myCheckbox').html();


        bodyTag.append($('<div id="main"/>').append(html));


        bodyTag.append($('#dialog')[0].outerHTML);


        $('#hit', bodyTag).remove();
        $('.hitarea', bodyTag).removeAttr("class");
        $('.last', bodyTag).removeAttr("class");
        $('.collapsable', bodyTag).removeAttr("class");
